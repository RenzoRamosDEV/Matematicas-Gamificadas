...
export function sha256HexJs(texto: string): string {
  ...
  const m = new Uint8Array(padLen); m.set(bytes); m[bytes.length] = 0x80;
  const dv = new DataView(m.buffer); dv.setUint32(padLen - 4, bitLen >>> 0); dv.setUint32(padLen - 8, Math.floor(bitLen / 2 ** 32));
